...
TCPClient                      client("127.0.0.1", 6841);
...
void MyThread::analyserData()
{
    ...
    if (it != radarmsgary.end())
    {
        ...
        int length = (static_cast<unsigned char>(radarmsgary[4]) << 24) | (static_cast<unsigned char>(radarmsgary[5]) << 16) | (static_cast<unsigned char>(radarmsgary[6]) << 8) |

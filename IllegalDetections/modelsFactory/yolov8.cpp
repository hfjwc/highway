...
const std::vector<std::vector<int>> colors = {{255, 0, 0},   {255, 85, 0},  {255, 170, 0},  {255, 255, 0},   {170, 255, 0},   {85, 255, 0},    {0, 255, 0},   {0, 255, 85},  {0, 255, 170},
                                              {0, 255, 255}, {0, 170, 255}, {0, 85, 255},   {0, 0, 255},     {85, 0, 255},    {170, 0, 255},   {255, 0, 255}, {255, 0, 170}, {255, 0, 85},
                                              {255, 0, 0},   {255, 0, 255}, {255, 85, 255}, {255, 170, 255}, {255, 255, 255}, {170, 255, 255}, {85, 255, 255}};
...
int YoloV8::Detect(const std::vector<bm_image>& input_images, std::vector<YoloV8BoxVec>& boxes)
{
    int ret = 0;
    // 3. preprocess
    // LOG_TS(m_ts, "start yolov8 preprocess");
    ret = pre_process(input_images);
    CV_Assert(ret == 0);
    // 4. forward
    // LOG_TS(m_ts, "start yolov8 inference");
    ret = m_bmNetwork->forward();
    CV_Assert(ret == 0);
    // 5. post process
    // LOG_TS(m_ts, "start yolov8 postprocess");
...
void YoloV8::draw_bmcv(bm_handle_t& handle, int classId, float conf, int left, int top, int width, int height, bm_image& frame,
                       bool put_text_flag)   // Draw the predicted bounding box
{
    ...
    if (put_text_flag)
    {
        ...
        // //rectangle(frame, Point(left, top - int(1.5 * labelSize.height)), Point(left + int(1.5 * labelSize.width),
        // top + baseLine), Scalar(0, 255, 0), FILLED); cv::putText(frame, label, cv::Point(left, top),

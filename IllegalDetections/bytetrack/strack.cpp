...
void STrack::activate(std::shared_ptr<Kalman::KalmanFilter> kalman_filter, int frame_id)
{
...
void STrack::re_activate(std::shared_ptr<Kalman::KalmanFilter> kalman_filter, std::shared_ptr<STrack> new_track, int frame_id, bool new_id)
{
...
void STrack::update(std::shared_ptr<Kalman::KalmanFilter> kalman_filter, std::shared_ptr<STrack> new_track, int frame_id)
{
    this->frame_id = frame_id;
    this->tracklet_len++;
    ...
    static_tlwh();
    static_tlbr();
    if (this->trajectory_blwh.size() > 200)
    {
        this->trajectory_blwh.pop_front();
    }
    this->trajectory_blwh.push_back(this->tlwh);
    this->state        = TrackState::Tracked;
    this->is_activated = true;
    this->score        = new_track->score;
    this->class_id     = new_track->class_id;
    this->det_tlwh     = new_track->tlwh;
}
...
void STrack::multi_predict(std::vector<std::shared_ptr<STrack>>& stracks, std::shared_ptr<Kalman::KalmanFilter> kalman_filter)
{

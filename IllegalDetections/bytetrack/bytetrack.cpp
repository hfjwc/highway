...
BYTETracker::~BYTETracker() {}
void BYTETracker::init(const bytetrack_params& params)
{
    ...
    this->kalman_filter = std::make_shared<Kalman::KalmanFilter>();
...
void BYTETracker::update(STracks& output_stracks, const std::vector<BoundingBox>& objects)
{
    ...
    for (int i = 0; i < objects.size(); i++)
    {
        ...
        std::vector<float> detTlwh;
        detTlwh.resize(4);
        detTlwh[0] = objects[i].x;
        detTlwh[1] = objects[i].y;
        detTlwh[2] = objects[i].width;
        detTlwh[3] = objects[i].height;

        float score    = objects[i].score;
        int   class_id = objects[i].class_id;

        std::shared_ptr<STrack> strack = std::make_shared<STrack>(STrack::tlbr_to_tlwh(tlbr_), score, class_id);
        strack->det_tlwh               = detTlwh;
    ...
    for (int i = 0; i < matches.size(); i++)
    {
        unconfirmed[matches[i][0]]->update(this->kalman_filter, detections[matches[i][1]], this->frame_id);
    ...
    joint_stracks(this->tracked_stracks, activated_stracks, this->tracked_stracks);
    ...
    {
        temp_removed_stracks[i]->lost_time = std::chrono::high_resolution_clock::now();
    ...
    for (int i = 0; i < this->tracked_stracks.size(); i++)
    {
        if (this->tracked_stracks[i]->is_activated && this->tracked_stracks[i]->tlwh[2] * this->tracked_stracks[i]->tlwh[3] > this->min_box_area)
...
void BYTETracker::joint_stracks(STracks& tlista, STracks& tlistb, STracks& results)
{
    ...
    for (int i = 0; i < results.size(); i++) exists.insert(std::pair<int, int>(results[i]->track_id, 1));
...
void BYTETracker::sub_stracks(STracks& tlista, STracks& tlistb)
{
    ...
    for (int i = 0; i < tlista.size(); i++) stracks.insert(std::pair<int, std::shared_ptr<STrack>>(tlista[i]->track_id, tlista[i]));
    for (int i = 0; i < tlistb.size(); i++)
    {
        int tid = tlistb[i]->track_id;
    ...
    tlista.clear();
    for (std::map<int, std::shared_ptr<STrack>>::iterator it = stracks.begin(); it != stracks.end(); ++it) tlista.push_back(it->second);
}

void BYTETracker::remove_duplicate_stracks(STracks& resa, STracks& resb, STracks& stracksa, STracks& stracksb)
{
...
void BYTETracker::linear_assignment(std::vector<std::vector<float>>& cost_matrix, int cost_matrix_size, int cost_matrix_size_size, float thresh, std::vector<std::vector<int>>& matches,
...
void BYTETracker::ious(std::vector<std::vector<float>>& atlbrs, std::vector<std::vector<float>>& btlbrs, std::vector<std::vector<float>>& results)
{
...
void BYTETracker::iou_distance(const STracks& atracks, const STracks& btracks, std::vector<std::vector<float>>& cost_matrix)
{
...
void BYTETracker::lapjv(const std::vector<std::vector<float>>& cost, std::vector<int>& rowsol, std::vector<int>& colsol, bool extend_cost, float cost_limit, bool return_cost)
{
